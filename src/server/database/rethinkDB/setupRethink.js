...
import databaseTable from './rethinkTables';

// ava is the test database
const databases = ['asteroidjs', 'ava'];

export default async function setupRethink(isUpdate = false) {
...
async function reset({db, isUpdate}) {
  ...
  await Promise.all(databaseTable.map(table => {
  ...
  const tableIndicies = await Promise.all(databaseTable.map(table => {
    return r.db(db).table(table.name).indexList().run();
  }));
  await Promise.all([...databaseTable.map((table, i) => {
